...
export function getJsonPath(file: string, dir = __dirname): string {
  return join(dir, `${file}.json`);
}
...
function jsonReplace(
  ...
  fileReplaceContents(getJsonPath(file, dir), search, replace);
}

export function replaceJsonRunCommand(file: string, dir: string) {
  jsonReplace(file, dir, /npm run/g, pm.runCommand());
}
...
export function getJson(file: string, dir: string): Record<string, any> {
  const packageJsonPath = getJsonPath(file, dir);

  return JSON.parse(fileGetContents(packageJsonPath));
}

export function setJson(file: string, dir: string, json: Record<string, any>) {
  putJson(getJsonPath(file, dir), json);
}
...
export function updatePackageName(newName: string, file = 'package', dir = __dirname): void {
  const cleanedName = validatePackageName(newName);
  const packageJson = getJson(file, dir);

  packageJson.name = cleanedName;
  setJson(file, dir, packageJson);
}
