...
interface AbiPlugin extends Plugin {
...
export const refreshPaths = ["src/**"].filter((path) =>
...
export default function v(
	...
	return [
		resolveAbiPlugin(pluginConfig),
		...(resolveFullReloadConfig(pluginConfig) as Plugin[]),
	];
}

/**
 * Resolve the Abi Plugin configuration.
 */
function resolveAbiPlugin(pluginConfig: Required<PluginConfig>): AbiPlugin {
	...
	return {
		...
		config: (config, { command, mode }) => {
			...
			return {
				...
				server: {
					origin:
						userConfig.server?.origin ?? "http://__abi_vite_placeholder__.test",
		...
		transform(code) {
			if (resolvedConfig.command === "serve") {
				code = code.replace(
					/http:\/\/__abi_vite_placeholder__\.test/g,
					viteDevServerUrl,
				);
				...
				if (isAddressInfo(address)) {
					...
					setTimeout(() => {
						server.config.logger.info(
							`\n  ${colors.red(`${colors.bold("Abi")} ${abiVersion()}`)}  ${colors.dim("plugin")} ${colors.bold(`v${pluginVersion()}`)}`,
						);
						server.config.logger.info("");
						server.config.logger.info(
...
function ensureCommandShouldRunInEnvironment(
	...
	if (command === "build" || env.ABI_BYPASS_ENV_CHECK === "1") {
		return;
	}

	if (typeof env.CI !== "undefined") {
		throw Error(
			"You should not run the Vite HMR server in CI environments. You should build your assets for production instead. To disable this ENAbi check you may set V_BYPASS_ENV_CHECK=1",
		);
	}
}

/**
 * The version of Abi being run.
 */
function abiVersion(): string {
	try {
		const abiPkg = require("abi.js/package.json");

		return abiPkg.version;
	} catch {
		return "";
	}
}
...
/**
 * The version of the Abi Vite plugin being run.
 */
...
function resolvePluginConfig(
	...
	if (typeof config === "undefined") {
		throw new Error("@abijs/vite: missing configuration.");
	}
	...
	if (typeof config.input === "undefined") {
		throw new Error('@abijs/vite: missing configuration for "input".');
	}
	...
	if (typeof config.publicDirectory === "string") {
		...
		if (config.publicDirectory === "") {
			throw new Error(
				"@abijs/vite: publicDirectory must be a subdirectory. E.g. 'public'.",
			);
		}
	}
	...
	if (typeof config.buildDirectory === "string") {
		...
		if (config.buildDirectory === "") {
			throw new Error(
				"@abijs/vite: buildDirectory must be a subdirectory. E.g. 'build'.",
			);
		}
	}
