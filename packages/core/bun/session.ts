...
export class SessionHandler {
    constructor(readonly path = `${import.meta.dir}/.sessions`, readonly prefix = 'session_') {
    ...
    write(id: string, data: SessionData) {
        const raw = Bun.inflateSync(serialize(data, { binaryType: "nodebuffer" })).toString()
...
export class Session {
    ...
    constructor(protected handler: SessionHandler, id?: string) {
    ...
    get<T>(key: string, defaultValue?: T): T | null | undefined {
        return this.data[key] !== undefined ? (this.data[key] satisfies T) : defaultValue
    }
